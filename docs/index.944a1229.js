 ...
 */class c3{constructor(e){this.apiKey=e}getGenerativeModel(e,t){if(!e.model)throw new cS("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new c2(this.apiKey,e,t)}getGenerativeModelFromCachedContent(e,t,n){if(!e.name)throw new cR("Cached content must contain a `name` field.");if(!e.model)throw new cR("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==t?void 0:t[n])&&e[n]&&(null==t?void 0:t[n])!==e[n]){if("model"===n&&(t.model.startsWith("models/")?t.model.replace("models/",""):t.model)===(e.model.startsWith("models/")?e.model.replace("models/",""):e.model))continue;throw new cR(`Different value for "${n}" specified in modelParams (${t[n]}) and cachedContent (${e[n]})`)}let r=Object.assign(Object.assign({},t),{model:e.model,tools:e.tools,toolConfig:e.toolConfig,systemInstruction:e.systemInstruction,cachedContent:e});return new c2(this.apiKey,r,n)}}let c6=!1;async function c5(){try{let e=await lk(aW(cI,"apikey","googlegenai"));if(e.exists()){let t=e.data().key;console.log("✅ Google Gemini API Key Loaded:",t),n=new c3(t).getGenerativeModel({model:"gemini-1.5-flash"}),c6=!0}else c4("\uD83D\uDEA8 No Google Gemini API key found in Firestore.")}catch(e){console.error("\uD83D\uDEA8 Error fetching API key:",e.message),c4("\uD83D\uDEA8 Chatbot error: API Key issue.")}}async function c8(e){if(!c6||!n){c4("\uD83D\uDEA8 AI is still initializing... Please wait.");return}try{c4(`\u{1F9D1}\u{200D}\u{1F4BB} You: ${e}`);let t=await n.generateContent({contents:[{role:"user",parts:[{text:e}]}]});if(console.log("\uD83D\uDFE1 AI Full Response:",t),!t?.candidates||0===t.candidates.length){console.warn("⚠️ AI did not generate a valid response."),c4("\uD83D\uDEAB AI could not generate a meaningful response.");return}let r=t.candidates[0]?.content?.parts?.map(e=>e.text).join(" ").trim();!r||r.length<5?(console.warn("⚠️ AI returned an empty or invalid response."),c4("\uD83D\uDEAB AI could not generate a meaningful response.")):c4(`\u{1F916} AI: ${r}`)}catch(e){console.error("\uD83D\uDEA8 Chatbot Error:",e),c4(`\u{1F6A8} Chatbot Error: ${e.message||"Could not reach AI"}`)}}function c4(e){let t=document.getElementById("chat-history");if(!t)return;let n=document.createElement("div");n.textContent=e,n.className="history",t.appendChild(n),t.scrollTop=t.scrollHeight}function c7(){let e=document.getElementById("chat-input");if(!e)return;let t=e.value.trim();t?c8(t):c4("⚠️ Please enter a prompt."),e.value=""}function c9(){localStorage.removeItem("email"),window.location.href="index.html"}function fe(){let e=document.getElementById("recipeInput").value.trim(),t=document.getElementById("categoryInput").value.trim(),n=document.getElementById("ingredientsInput").value.trim().split(",");e&&t&&n.length>0?ft(e,t,n):alert("\uD83D\uDEA8 Please fill out all fields.")}async function ft(e,t,n){let r=JSON.parse(localStorage.getItem("email"));if(!r){alert("You must be logged in to add recipes.");return}try{await function(e,t){var n;let r=aB(e.firestore,aJ),i=aW(e),s=(n=e.converter)?n.toFirestore(t):t;return lx(r,[(function(e,t,n,r,i,s={}){let o,a;let l=e.ju(s.merge||s.mergeFields?2:0,t,n,i);ls("Data must be an object, but it was:",l,r);let u=lr(r,l);if(s.merge)o=new n8(l.fieldMask),a=l.fieldTransforms;else if(s.mergeFields){let e=[];for(let r of s.mergeFields){let i=lo(t,r,n);if(!l.contains(i))throw new nT(nb.INVALID_ARGUMENT,`Field '${i}' is specified in your field mask but missing from your input data.`);lh(e,i)||e.push(i)}o=new n8(e),a=l.fieldTransforms.filter(e=>o.covers(e.field))}else o=null,a=l.fieldTransforms;return new a5(new rP(u),o,a)})(le(e.firestore),"addDoc",i._key,s,null!==e.converter,{}).toMutation(i._key,iO.exists(!1))]).then(()=>i)}(aG(cI,"recipes"),{name:e,category:t,ingredients:n,email:r,favorite:!1,created_at:new Date}),fn()}catch(e){console.error("\uD83D\uDEA8 Error adding recipe:",e)}}async function fn(){let e=JSON.parse(localStorage.getItem("email"));if(!e)return;let t=lm(aG(cI,"recipes"),lv("email","==",e)),n=await lN(t),r=document.getElementById("recipeList");r.innerHTML="",n.forEach(e=>{let t=e.data(),n=document.createElement("li");n.innerHTML=`
    ...
    `,r.appendChild(n)})}async function fr(){let e=document.getElementById("ingredientFilter").value.toLowerCase(),t=document.getElementById("categoryFilter").value.toLowerCase(),n=lm(aG(cI,"recipes"),lv("email","==",JSON.parse(localStorage.getItem("email")))),r=await lN(n),i=document.getElementById("recipeList");if(i.innerHTML="",r.forEach(n=>{let r=n.data();if((!e||r.ingredients.some(t=>t.toLowerCase().includes(e)))&&(!t||r.category.toLowerCase().includes(t))){let e=document.createElement("li");e.innerHTML=`
      ...
      `,i.appendChild(e)}}),document.querySelectorAll(".delete-btn").forEach(e=>{e.addEventListener("click",()=>deleteRecipe(e.dataset.id))}),document.querySelectorAll(".edit-btn").forEach(e=>{e.addEventListener("click",()=>editRecipe(e.dataset.id))}),document.querySelectorAll(".favorite-btn").forEach(e=>{e.addEventListener("click",()=>toggleFavorite(e.dataset.id))}),e||t){let e=document.createElement("button");e.textContent="Reset Filters",e.addEventListener("click",()=>{document.getElementById("ingredientFilter").value="",document.getElementById("categoryFilter").value="",fn()}),i.appendChild(e)}}document.addEventListener("DOMContentLoaded",()=>{document.getElementById("send-btn")?.addEventListener("click",c7)}),document.addEventListener("DOMContentLoaded",async()=>{document.getElementById("send-btn")?.addEventListener("click",()=>{let e=document.getElementById("chat-input").value.trim();e&&c8(e)}),document.getElementById("addRecipeBtn")?.addEventListener("click",fe),document.getElementById("signOutBttn")?.addEventListener("click",c9),document.getElementById("filterBtn")?.addEventListener("click",fr),await c5(),fn()}),window.deleteRecipe=async function(e){var t;await lx(aB((t=aW(cI,"recipes",e)).firestore,aJ),[new i$(t._key,iO.none())]),fn()},window.editRecipe=async function(e){let t=prompt("Enter new recipe name:"),n=prompt("Enter new category:"),r=prompt("Enter new ingredients (comma-separated):");t&&n&&r?(await lO(aW(cI,"recipes",e),{name:t,category:n,ingredients:r.split(",")}),fn()):alert("\uD83D\uDEA8 Please fill in all fields.")},window.toggleFavorite=async function(e){let t=aW(cI,"recipes",e),n=(await lk(t)).data().favorite||!1;await lO(t,{favorite:!n}),fn()};const fi="recipe-organizer-cache-v4",fs=["./html/index.html","./html/tasks.html","./css/style.css","./app.js","./manifest.json"];self.addEventListener("install",e=>{e.waitUntil(caches.open(fi).then(e=>(console.log("[Service Worker] Pre-caching offline content"),e.addAll(fs)))),self.skipWaiting()}),self.addEventListener("activate",e=>{e.waitUntil(caches.keys().then(e=>Promise.all(e.map(e=>{if(e!==fi)return console.log("[Service Worker] Removing old cache:",e),caches.delete(e)})))),self.clients.claim()}),self.addEventListener("fetch",e=>{e.respondWith(caches.match(e.request).then(t=>t||fetch(e.request).catch(()=>caches.match("./html/index.html"))))});const fo=JSON.parse(localStorage.getItem("email"));window.location.pathname.includes("index.html")&&fo&&(window.location.href="tasks.html"),window.location.pathname.includes("tasks.html")&&!fo&&(window.location.href="index.html");var fa={};fa=new URL("up_/js/service-worker.js",import.meta.url).toString(),"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register(fa).then(e=>console.log("✅ Service Worker Registered:",e)).catch(e=>console.error("\uD83D\uDEA8 Service Worker Registration Failed:",e))});
//# sourceMappingURL=index.944a1229.js.map
