...
// ✅ Delete Recipe (Fix: Expose Globally)
window.deleteRecipe = async function (recipeId) {
  await deleteDoc(doc(db, "recipes", recipeId));
  getRecipes();
};

// ✅ Edit Recipe (Fix: Expose Globally)
...
// ✅ Toggle Favorite Recipe (Fix: Expose Globally)
...
async function addRecipe(name, category, ingredients) {
  ...
  try {
    ...
    setTimeout(() => {
      document.getElementById(docRef.id)?.classList.add("feedback-highlight");
    }, 500);
  } catch (error) {
    console.error("🚨 Error adding recipe:", error);
...
document.addEventListener("DOMContentLoaded", async () => {
  setupEventListeners();
  await getApiKey();
  getRecipes(); // ✅ Recipes now load IMMEDIATELY after signing in!
});
...
async function addRecipe(name, category, ingredients) {
  ...
  try {
    ...
    getRecipes();
    showFeedbackMessage("✅ Recipe Added!");
    resetRecipeInputs(); // ✅ Clears Input Fields After Adding
  } catch (error) {
    console.error("🚨 Error adding recipe:", error);
    showFeedbackMessage("🚨 Error adding recipe!", "error");
  }
...
// ✅ Get Recipes Immediately After Sign-in
async function getRecipes() {
  const email = JSON.parse(localStorage.getItem("email"));
  if (!email) return;
  ...
  ensureResetButton(); // ✅ Make sure Reset Button is always there
}
...
async function getRecipes() {
  ...
  ensureResetButton(); // ✅ Ensure Reset Button is Always There
}
...
async function addRecipe(name, category, ingredients) {
  ...
  try {
    ...
    getRecipes();
    resetRecipeInputs(); // ✅ Clear Input Fields After Adding Recipe
    showFeedbackMessage("✅ Recipe Added!");
  } catch (error) {
    console.error("🚨 Error adding recipe:", error);
    showFeedbackMessage("🚨 Error adding recipe!", "error");
...
async function getRecipes() {
  ...
  ensureResetFilterButton(); // ✅ Ensure Reset Button Appears
}

// ✅ Filter Recipes (Fixed)
async function filterRecipes() {
  const ingredientFilter = document
    .getElementById("ingredientFilter")
    .value.toLowerCase();
...
// ✅ Ensure Reset Filters Button Appears
...
// ✅ Create Recipe Item (Fix: Ensure Recipe Formatting)
