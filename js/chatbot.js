...
export async function askChatBot(userInput) {
  ...
  try {
    ...
    // **Use clear user instructions instead of system role**
    const formattedRequest = `
    This is a chatbot for a **Recipe Organizer app**. 
    - Users can add, edit, delete, and filter recipes. 
    - If a user asks about recipes, give **specific steps**. 
    ...
    **User's question:** ${userInput}
    ...
    const result = await model.generateContent({
      contents: [{ parts: [{ text: formattedRequest }] }],
    });
...
export async function askChatBot(userInput) {
  ...
  try {
    ...
    // ✅ Formatted AI request with clear instructions
    const formattedRequest = `
    This is a chatbot for a **Recipe Organizer app**. 
    - Users can add, edit, delete, and filter recipes. 
    - If a user asks about recipes, give **specific steps**. 
    ...
    **User's question:** ${userInput}
    ...
    // ✅ Extract AI response correctly (Handles different response structures)
    let aiResponse = "";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ Ensure Toggle Button is Styled Correctly
  toggleButton.style.color = "#ffffff";
  toggleButton.style.fontSize = "28px"; // 🔥 Increase Symbol Size
